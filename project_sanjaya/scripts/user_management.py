...
def change_username(args):
    ...
    response = requests.put(f"{API_URL}/users/{args.username}/change-username", json={"new_username": args.new_username}, headers=get_headers())
    if response.status_code == 200:
        print(f"Username for user '{args.username}' changed to '{args.new_username}' successfully.")
    else:
        print(f"Error changing username: {response.json()['detail']}")
...
def main():
    ...
    change_username_parser = subparsers.add_parser("change-username", help="Change a user's username")
    change_username_parser.add_argument("username", help="The current username of the user")
    change_username_parser.add_argument("new_username", help="The new username")
    change_username_parser.set_defaults(func=change_username)
